...
SELECT COUNT(DISTINCT CUSTOMER_ID) AS TOTAL_UNIQUE_CUSTOMER FROM RETAIL;
...
2. **Set Up the Database**: Run the SQL scripts provided in the `sql_all_queries.sql` file to create and populate the database.
3. **Run the Queries**: Use the SQL queries provided in the `sql_all_queries.sql` file to perform your analysis.
